...
<div class="mb-4">
    ...
    <div class="flex items-center gap-3">
        <input type="color" id="field-{{ $field->name }}" wire:model="data.{{ $field->name }}"
            @disabled($field->isDisabledForAction($action ?? null))
            class="h-11 w-14 shrink-0 cursor-pointer rounded-lg border border-gray-300 bg-transparent p-1 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-700">
        <input type="text" wire:model="data.{{ $field->name }}"
            @disabled($field->isDisabledForAction($action ?? null))
            placeholder="#000000"
