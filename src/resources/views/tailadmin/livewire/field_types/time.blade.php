{{--
    HTML5 time input — the "time only" counterpart datetime.blade.php never
    got a sibling for (only date+time together, or date alone via the
    'date' alias to birthday.blade.php).
--}}
@php
...
<div class="mb-4">
    ...
    <input type="time" id="field-{{ $field->name }}" wire:model="data.{{ $field->name }}"
