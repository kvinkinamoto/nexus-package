{{--
    'url' already had a default validation rule (registerBuiltInFieldTypeDefaultRules())
    but no view — a #[Field(type: 'url')] fell straight through to the
    unsupported banner. Same shape as email.blade.php.
--}}
@php
...
<div class="mb-4">
    ...
    <div class="relative">
        <i class="{{ nexus_icon('link') }} pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400"></i>
        <input type="url" id="field-{{ $field->name }}" wire:model="data.{{ $field->name }}"
            placeholder="https://" @disabled($field->isDisabledForAction($action ?? null))
            class="{{ $inputClass }}">
    </div>
